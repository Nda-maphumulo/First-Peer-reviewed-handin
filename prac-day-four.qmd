...
## Installing packages then the answers will follow.
```{r}
#| echo: false
...
#| echo: false
data("flights", package = "nycflights13") #manually loading this to environment
```
...
```{r}
#=================================================================#
# Displaying DataSet using tibble                                 #
#=================================================================#
as_tibble(flights)
```
...
```{r}
#Keeps rows of January, month 1.
flights_jan <- flights |> filter(month == 1)

#Returns unique rows of carrier names.
unique_carriers <- flights |> distinct(carrier)

# Calculate mean and standard deviation of distance by carrier
...
table <- tibble(
  ...
  mean_distance = carrier_summary$mean_distance,
  sd_distance = carrier_summary$sd_distance
)

as_tibble(table)

#Rearranging the data frame
table |> arrange(mean_distance)
  
```

## Question 3

Answering the question why is sd 0. There must be an observation equal to it mean making the deviation 0.

```{r}
library(dplyr)
library(nycflights13)

# Find flights where distance equals the mean distance for that carrier
flights |> 
  group_by(carrier) |> 
...
Answering the question why is sd NA. There must be an ABSENT observation making the deviation NA.

```{r}
# Find rows where the distance is NA
flights |> 
...
Installing Packages first for question 4
...
# Calculate the proportion of delayed departures but on-time arrivals
proportion <- flights |> 
  filter(dep_delay > 0 & arr_delay <= 0) |>  # Filter flights with delayed departure and on-time arrival
  summarise(proportion = n() / nrow(flights))  # Calculate the proportion of such flights
...
(i) Identifying routes that more than one airline flies
...
routes_multiple_airlines <- flights |> 
  ...
  summarise(airline_count = n_distinct(carrier)) |> 
  filter(airline_count > 1)
...
```{r}
# Calculate the average arrival delay for each airline on routes with more than one airline
average_delay_per_airline <- flights |> 
...
(iii)For each such route, identifying the airline with the worst and best average arrival delay.

```{r}
# Identify the worst and best average arrival delay airlines for each route
best_worst_airlines <- average_delay_per_airline |> 
  group_by(origin, dest) |> 
  summarise(
    best_airline = carrier[which.min(avg_arrival_delay)],
    worst_airline = carrier[which.max(avg_arrival_delay)],
    best_delay = min(avg_arrival_delay),
    worst_delay = max(avg_arrival_delay)
  )

# Display the result
print(best_worst_airlines)
```

(iv). Identifying the route with the greatest difference between the best and worst performing airlines.

```{r}
# Calculate the difference between the best and worst performing airlines for each route
route_with_greatest_difference <- best_worst_airlines |> 
  mutate(delay_diff = worst_delay - best_delay) |> 
  arrange(desc(delay_diff)) |> 
  slice(1)

# Display the result
print(route_with_greatest_difference)
```

(v). This could be the result of the operational issue.

## Question 7
```{r}
#| echo: false
# Load the data and name it Q7_data
...
```
```{r}
# dput() output as a data frame
Q7_data
```

Dowloading a GitHub repo
```{r}
#| echo: false
...
```
Inspecting the unique values in each columns
```{r}
#| echo: false
UtilsDataRSV::view_cols(Q7_data)
```
There is a typo in row 10. Instead of healthy there is Healthy with capital letter.